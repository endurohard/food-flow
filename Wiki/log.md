...
## [2026-09-02] feat | Касса, роль кассира, отключение витрины и чистка палитры (коммиты eed2377…82b1d32)

Четыре связанных изменения одной сессии. Все на проде **кроме** бэкенда роли кассира.

**Публичная витрина отключена** (`eed2377`). `customer-app/index.html` (1169 строк B2C-меню) заменён редиректом, корневой редирект `frontend-service` переведён с витрины на логин. Новый `frontend/index.html` — точка входа: залогиненного ведёт в админку (super_admin → своя панель, cashier → касса), остальных на `login.html`. Лежит в корне статики, поэтому `express.static` отдаёт его раньше серверного роута — работает без пересборки образа (`frontend` примонтирован томом).

**Кассовый экран** `admin-panel/pos.html` (`4e03a4f`). Следствие отключения витрины: `POST /api/orders` не вызывала **ни одна** страница админки — заказ было негде создать. Экран: меню плитками, поиск, категории, стоп-лист заблокирован, модификаторы группами (`modifier_group`, `price_adjustment`), чек, тип заказа, наличные со сдачей и карта, печать чека 72 мм. Проводка: `POST /api/orders` → `PUT /:id/status confirmed` → `POST /api/finance/payments` → для наличных `POST /registers/:id/operations` (`sale`). Ссылка «Касса» и доступ ролям waiter/operator во всех страницах; `MODULE_ACCESS` разрешает `pos.html` типам restaurant/cafe/coffee_shop.

**Роль `cashier`** (`dbe5849`). Миграция 027 — `cashier` в CHECK на `enterprise_users.role`. Новый `POST /api/enterprises/:id/staff` (`enterprise.routes.ts`) создаёт пользователя и привязку к организации в транзакции: раньше добавить сотрудника можно было только зная UUID уже зарегистрированного пользователя (`POST /:id/users`), то есть завести бариста из панели было нельзя. Права: super_admin сквозной, иначе owner/admin **этой** организации — сверка по членству, а не по `:id` из URL (в старом `POST /:id/users` такой сверки нет — владелец одной организации формально может добавить юзера в чужую). Кассир видит в сайдбаре только кассу и попадает в неё сразу после логина.

**Хвосты старой палитры** (`82b1d32`). Тема «Базилик и томат» стоит с `dedcecb`, но `#00B074` (Sedap) оставался в фавиконе-data-URI, в `stroke` иконок аватара на каждой странице, в градиенте брендовой панели `login.html` и в цветах графиков дашборда. Вынесен общий `assets/favicon.svg`, иконки переведены на `currentColor` + `--color-primary` у контейнера, графики читают токены через `getComputedStyle`.

### Находки по ходу
- **`idempotencyCheck` кэширует и неуспешные ответы** (`services/*/src/middleware/idempotency.middleware.ts:34` — `res.json` перехватывается до проверки статуса). Повтор запроса с тем же ключом после 400 вечно возвращает ту же ошибку. В `pos.html` из-за этого фолбэк-платёж уходит с новым ключом, а прежний ключ переиспользуется только после сетевого сбоя.
- **`/api/menus/*` — мёртвый роут**: Kong маршрутизирует `/api/menus` на restaurant-service (`kong/kong.yml:27`), но сервис монтирует только `/api/restaurants` и `/api/reservations` (`services/restaurant-service/src/index.ts:53`). `menu.html` зовёт `/api/menus/stop-list` → 404. Касса берёт стоп-лист из полей `menu_items` (`stopped_at`/`stop_until`).
- **`GET /api/restaurants` — `optionalAuth`**: с валидным токеном список скоупится по `req.enterpriseId`, без токена отдаёт все публичные заведения. Касса всегда ходит с токеном, поэтому берёт заведение своей организации.
- **На проде лежали 14 файлов вне git** (ноя–дек 2025): `pos-terminal.html`, `menu-management.html`, `users.html`, `waybills.html`, `production-orders.html`, `enterprise-settings.html`, `register.html`, `common-styles.css`, четыре скрипта `admin-panel/js/` и `customer-app/{qr-menu.html,js/branding.js}`. Ни один не подключает `tokens.css`, ссылок из живых страниц нет. `pos-terminal.html` — полный мок: захардкоженные товары, `fetch` не вызывается ни разу, «оплата» это `setTimeout` + `alert('Заказ успешно оформлен')`, авторизации нет. Бэкап: `~/frontend-orphans-2026-09-02.tgz` на боксе.

### Деплой
Фронт разложен на бокс файлами (том `./frontend:/app/public:ro` — пересборка не нужна). Бэкенд: **у контейнеров нет интернета, образ на боксе не собрать**, поэтому правки `finance-service` доставлены подменой скомпилированного `dist` (`docker cp` + рестарт, рядом `.bak`). Подмена не переживёт `--force-recreate`; `.ts`-исходники на боксе обновлены, ближайшая пересборка закрепит. **Не выполнено на 2026-09-02**: миграция 027 и `dist` для роли `cashier` (`user-service` + `finance-service`) — команды подготовлены, файлы лежат в `/tmp` на боксе.

## [2026-09-03] fix | Идемпотентность вешала запрос: crm/finance без REDIS_URL (коммит 5babe16)

Найдено при сквозном прогоне продажи от лица кассира на проде.

**Симптом**: `POST /api/finance/payments` с заголовком `idempotency-key` не отвечает вообще (60+ с, в логах только строка о приёме запроса, ошибок нет). Тот же запрос без заголовка — 201 за 1.2 с.

**Причина**: `docker-compose.yml` не задавал `REDIS_URL` для `crm-service` и `finance-service` (у `order-service` он был — там идемпотентность работала). `config.redis.url` падал на дефолт `redis://localhost:6379`, внутри контейнера там пусто, node-redis уходил в бесконечный реконнект, `await client.connect()` в `idempotency.middleware.ts` не резолвился и держал запрос навсегда. Задеты все идемпотентные роуты этих двух сервисов: платежи (касса шлёт ключ всегда), промокоды и списание баллов в crm.

**Починка**: `REDIS_URL` в compose обоим сервисам + в `idempotency.middleware.ts` всех трёх сервисов таймауты (connect 2 с, get 1 с), `reconnectStrategy: false` и разовая пометка «Redis недоступен» — идемпотентность деградирует до no-op, но сервис не залипает. На проде env добавлен через `docker-compose.override.yml` (бэкап `docker-compose.override.yml.bak-redis`); пересоздание контейнеров сбрасывает подменённый `dist`, поэтому патчи возвращаются тем же скриптом после `compose up`.

**Проверено на проде после выкатки роли кассира**: `POST /api/enterprises/:id/staff` → 201; кассир входит, в JWT `enterpriseRole: cashier` + `enterpriseId`; создаёт заказ (`ORD-20260903-97971F`), переводит в `confirmed`, пишет платёж со `status: completed`; повтор оплаты с тем же ключом идемпотентности возвращает **тот же** платёж, дубля нет; владелец предприятия видит оба платежа в `GET /api/finance/payments`. Тестовый кассир отключён (`isActive: false`, вход → 403 «Account is deactivated»), тестовые заказы остались в демо-предприятии `00000000-…-0001`.

**Мелочь оттуда же**: order-service отдаёт сумму заказа в поле `total` (не `total_amount`) — `pos.html` поправлен, иначе при скидке чек разошёлся бы с заказом. Joi в `/staff` не принимает несуществующие TLD (`@foodflow.local` → 400), для служебных аккаунтов нужен реальный домен.
