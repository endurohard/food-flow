...
app.get('/health', async (_req: Request, res: Response) => {
  try {
    ...
    res.status(503).json({ status: 'unhealthy', service: 'finance-service', error: 'database unreachable' });
  }
});
...
const server = app.listen(PORT, () => { logger.info(`Finance service listening on port ${PORT}`); });
