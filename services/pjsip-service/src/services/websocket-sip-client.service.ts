...
import { UserAgent, Registerer, Inviter, Session, SessionState, Web, Transport } from 'sip.js';
import { SIPConfig, SIPUser, Call, CallStatus } from '../models/call.model.js';
import { logger } from '../utils/logger.js';
import { v4 as uuidv4 } from 'uuid';

/**
 * WebSocket-based SIP client using sip.js
 * Requires WebSocket URL (ws:// or wss://)
 */
export class WebSocketSIPClientService extends EventEmitter {
  ...
  async connect(): Promise<void> {
    try {
      logger.info('Connecting to SIP server via WebSocket...');
      ...
      logger.info(`Successfully connected ${this.config.users.length} SIP users via WebSocket`);
      this.emit('connected');
    } catch (error) {
      logger.error('Failed to connect to SIP server via WebSocket:', error);
  ...
  private async registerUser(user: SIPUser): Promise<void> {
    ...
    // Use WebSocket password if available, otherwise fall back to regular password
    const password = user.wsPassword || user.password;
    ...
    logger.info(`SIP user registered via WebSocket: ${user.username} (${user.extension})`);
  }
  ...
  private handleIncomingCall(invitation: any, user: SIPUser): void {
    ...
    logger.info(`Incoming call (WebSocket): ${from} -> ${to}`);
  ...
  async makeCall(from: string, to: string, autoAnswer: boolean = false): Promise<Call> {
    ...
    logger.info(`Outbound call initiated (WebSocket): ${from} -> ${to}`);
  ...
  async disconnect(): Promise<void> {
    logger.info('Disconnecting SIP users (WebSocket)...');
    ...
    logger.info('Disconnected from SIP server (WebSocket)');
  }
}
